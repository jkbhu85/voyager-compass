...
public class TravelMasterDao {
	...
	public boolean saveTravel(Travel travel) {
		...
		try {
			...
			pstmt.setString(col++, DateUtils.convertDate(travel.getStartDate()));
			pstmt.setString(col++, DateUtils.convertDate(travel.getEndDate()));
	...
	public List<Travel> findAllTravels() {
		...
		try {
			...
			while (rs.next()) {
				...
				travel.setStartDate(DateUtils.getDateString(rs.getDate("TRAVELSTARTDATE")));
				travel.setEndDate(DateUtils.getDateString(rs.getDate("TRAVELENDDATE")));
